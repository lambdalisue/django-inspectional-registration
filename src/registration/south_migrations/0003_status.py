...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for profile in orm.RegistrationProfile.objects.all():
            if profile.user.is_active:
                # the registration profile is no longer required
                profile.delete()
    ...
    def backwards(self, orm):
        ...
        for profile in orm.RegistrationProfile.objects.all():
            if profile._status == 'rejected':
                # the profile has rejected thus set the profile expired
                profile.user.date_joined -= datetime.timedelta(days=settings.ACCOUNT_ACTIVATION_DAYS+1)
                profile.user.save()
